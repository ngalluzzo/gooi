...
/**
 * Executes one webhook ingress payload through surface dispatch/binding and runtime invocation.
 */
export const runWebhookSurface = async (input: {
	...
	invokeSurfaceIngress({
		surfaceId: input.surfaceId ?? "webhook",
...
/**
 * Creates a reusable webhook adapter executor with fixed runtime/bundle context.
 */
export const createWebhookSurfaceAdapter = (input: {
	readonly runtime: AppRuntime;
	readonly bundle: SurfaceAdapterBundle;
	readonly surfaceId?: string;
}) => {

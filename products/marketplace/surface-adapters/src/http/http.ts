...
/**
 * Executes one HTTP ingress payload through surface dispatch/binding and runtime invocation.
 */
export const runHttpSurface = async (input: {
	...
	invokeSurfaceIngress({
		surfaceId: input.surfaceId ?? "http",
...
/**
 * Creates a reusable HTTP adapter executor with fixed runtime/bundle context.
 */
export const createHttpSurfaceAdapter = (input: {
	readonly runtime: AppRuntime;
	readonly bundle: SurfaceAdapterBundle;
	readonly surfaceId?: string;
}) => {

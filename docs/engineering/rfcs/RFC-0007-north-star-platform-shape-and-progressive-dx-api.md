...
1. Define the end-state product architecture across authoring, runtime, quality, and marketplace.
...
3. Runtime behavior remains deterministic and portable across surfaces and hosts.
...
Define the end-state platform as four product lanes backed by a progressive API ladder.
...
2. `Runtime`:
   - Entrypoint orchestration, provider activation/invocation, host-port integration.
   - Executes contracts deterministically across surfaces.
   - Resolves capability reachability (`local` vs `delegated`) from deployment artifacts.
3. `Marketplace`:
...
- Kernel/domain runtime:
  - Own policy gate and entrypoint orchestration semantics.
  - Must remain host/provider/surface agnostic.
...
1. Risk: Facade APIs become a hidden monolith.
   - Mitigation: Require parity tests and direct lower-level API equivalence.
